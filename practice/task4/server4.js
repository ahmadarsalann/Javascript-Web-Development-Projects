...
app.get('/search', async (req, res) => {
	...
	// TODO
	try {
		const term = "select * from campgrounds where name = $1;"
		const response = await pool.query(term, [searchTerm]);
		const results = response.rows.map(function(item) {
			fin = {campground: item.name, location: item.location, maxlength: item.maxlength};
			last[y] = fin;
			y++;
		})
		res.json({campgrounds: last});
...
app.get('/fit', async (req, res) => {
	...
	// TODO
	try {
		const term = "select * from campgrounds where maxlength > $1;"
		const response = await pool.query(term, [searchTerm]);
		const results = response.rows.map(function(item) {
			fin = {campground: item.name, location: item.location, maxLength: item.maxlength};
			last[y] = fin;
...
app.get('/elevation', async (req, res) => {
	...
	try {
		// TO DO
		const response = await pool.query(query, [num]);
		const results = response.rows.map(function(item) {
			fin = {campground: item.name, elevation: item.elevation, town: item.location};
			last[y] = fin;
			y++;
		})
		res.json({campgrounds: last});
...
app.post("/addCampground", async (req, res) => {
	...
	const height = req.body.elevation;
	const site = req.body.sites;
	try{
		const template1 = "select * from campgrounds where name = $1 and location = $2 and maxlength = $3 and elevation = $4 and sites = $5;"
		const check = await pool.query(template1, [name, town, length, height, site]);
		if(check.rowCount != 0){
			...
			const response = await pool.query(template2, [name, town, length, height, site]);
			res.json({status: "added"});
		}
	} catch (err){
		console.log(err);
